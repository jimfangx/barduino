...
class BarduinoChipTop(implicit p: Parameters) extends LazyModule with HasChipyardPorts {
  ...
  class BarduinoChipTopImpl extends LazyRawModuleImp(this) {
    ...
    val sckIOs = IOCell.generateFromSignal(system.qspi(0).sck, spiPorts.sck, Some("qspi_0_sck"), p(IOCellKey), IOCell.toAsyncReset)
    val csIOs = IOCell.generateFromSignal(system.qspi(0).cs, spiPorts.cs, Some("qspi_0_cs"), p(IOCellKey), IOCell.toAsyncReset)
